...
class LB_Actions {
	public function __construct(  ) {
		add_action( 'wp_ajax_hnmgbox_get_oembed', array( $this, 'get_oembed_ajax' ) );
		add_action( 'wp_ajax_nopriv_hnmgbox_get_oembed', array( $this, 'get_oembed_ajax' ) );
		add_action( 'wp_ajax_hnmgbox_get_items', array( $this, 'get_items_ajax' ) );
		add_action( 'wp_ajax_nopriv_hnmgbox_get_items', array( $this, 'get_items_ajax' ) );
	}
	...
	public function get_oembed_ajax(){
		...
		if( ! wp_verify_nonce( $_POST['ajax_nonce'], 'hnmgbox_ajax_nonce' ) ){
			die();
		}
		...
		$oembed = LB_Functions::get_oembed( $oembed_url, $preview_size );
		wp_send_json( $oembed );
	}

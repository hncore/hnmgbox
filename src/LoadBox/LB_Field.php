...
class LB_Field {
    ...
    public function get_hnmgbox() {
        return LB_LoadBox::get( $this->hnmgbox_id );
	}

    public function __get( $property ) {
        if( $property == 'hnmgbox' ){
            return $this->get_hnmgbox();
        }
        return null;
    }
	...
	public function set_args( $field_args = array() ){
		...
		switch( $field_type ){
			case 'button':
				$this->args['options'] = wp_parse_args( $this->args['options'], array(
					...
					'icon' => ''//<i class="hnmgbox-icon hnmgbox-icon-plus"></i>//font awesome icons
				));
				break;
			...
			case 'file':
				$this->args['options'] = array_replace_recursive( array(
					...
					'upload_file_text'     => __( 'Upload file', 'hnmgbox' ),
					'upload_file_class'    => ''
				), $this->args['options'] );

				if( array_intersect( array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ),
					$this->args['options']['mime_types'] ) ){
					$this->args['options']['upload_file_text'] = __( 'Upload image', 'hnmgbox' );
				}
				if( array_intersect( array( 'mp4', 'webm', 'ogv', 'ogg', 'vp8' ),
					$this->args['options']['mime_types'] ) ){
					$this->args['options']['upload_file_text'] = __( 'Upload video', 'hnmgbox' );
			...
			case 'group':
				$this->args['options'] = array_replace_recursive( array(
					'add_item_text'  => _x( 'Add new', 'New item for groups', 'hnmgbox' ),
					'remove_item_text' => _x( 'Remove', 'Remove item for groups', 'hnmgbox' ),
					'sortable' => true,
				), $this->args['options'] );
				...
				$this->args['controls'] = array_replace_recursive( array(
					...
					'images' => false,
					'default_image' => HNMGBOX_URL.'/img/transparent.png',
					'image_field_id' => '',
					'position' => 'top',//top, left
					'width' => '',
					'height' => '',
					'left_actions'  => array(
						'hnmgbox-info-order-item' => '#'
					),
					'right_actions'  => array(
						'hnmgbox-duplicate-group-item' => '<i class="dashicons dashicons-admin-page"></i>',
						'hnmgbox-remove-group-item' => '<i class="hnmgbox-icon hnmgbox-icon-trash"></i>'
					),
				), $this->args['controls'] );
				break;
			...
			case 'import':
				...
				$this->args['options'] = wp_parse_args( $this->args['options'],  array(
					...
					'in_line'			       => true,
					'like_checkbox'			 => false,
					'import_button_text' => __( 'Import', 'hnmgbox' ),
					'label_text_auth_fields' => __( 'Is your website password protected?', 'hnmgbox' ),
					'desc_text_auth_fields' => __( '(Optional)', 'hnmgbox' ),
				));
				//$this->args['import_data'] = wp_parse_args( $this->args['import_data'],  array() );

				$this->args['items'] = wp_parse_args( array(
					'from_file' => _x( 'From file', 'Import/Export field', 'hnmgbox' ),
					'from_url' => _x( 'From url', 'Import/Export field', 'hnmgbox' )
				) , $this->args['items']);
			...
			case 'export':
				$this->args['options'] = wp_parse_args( $this->args['options'],  array(
					'export_button_text'  => __( 'Download', 'hnmgbox' ),
                    'export_file_name' => 'hnmgbox-backup-file-'.$this->get_hnmgbox()->id,
				));
				break;
			...
			case 'oembed':
				$this->args['options'] = array_replace_recursive( array(
					...
					'get_preview_text' => __( 'Get preview', 'hnmgbox' )
				), $this->args['options'] );
				break;
			...
			case 'section':
				$this->args['options'] = wp_parse_args( $this->args['options'],  array(
					...
					'toggle_speed' => 400,
					'toggle_open_icon' => 'hnmgbox-icon-chevron-up',
					'toggle_close_icon' => 'hnmgbox-icon-chevron-down',
				));
				break;
		...
		if( $field_type == 'group' || $this->args['repeatable'] ) {
			$this->args['options'] = wp_parse_args( $this->args['options'],  array(
				'sortable' => true,
				'add_item_text' => _x( 'Add new', 'New item for repeatable fields', 'hnmgbox' ),
				'remove_item_text' => _x( 'Remove', 'Remove item for repeatable fields', 'hnmgbox' ),
				'add_item_class' => '',//'hnmgbox-custom-add' para agregar como nombre de cada control el texto del botón
				'remove_item_class' => '',
				'duplicate_item_text'  => __( 'Duplicate', 'hnmgbox' ),
				'duplicate_item_class' => '',
				'sort_item_text'  => __( 'Sort', 'hnmgbox' ),
				'sort_item_class' => '',
				'visibility_item_text'  => __( 'Visibility', 'hnmgbox' ),
	...
	public function validate_colorpicker( $color = '' ){
		$format_color = LB_Functions::get_format_color( $color );
		$format = $this->arg( 'options', 'format' );
		if( $format_color ){
			if( $format == 'hex' && ( $format_color == 'rgb' || $format_color == 'rgba' ) ){
				$color = LB_Functions::rgb_to_hex( $color );
			}
			if( ($format == 'rgb' || $format == 'rgba') && ( $format_color == 'hex' ) ){
				$color = LB_Functions::hex_to_rgb( $color, 1 );
			}
		} else {
			$color = '';
		}
	...
	public function add_field( $field_args = array() ){
		return $this->get_hnmgbox()->add_field( $field_args, $this );
	}

	public function get_field( $field_id = '' ){
		return $this->get_hnmgbox()->get_field( $field_id, $this );
	}

	public function add_group( $field_args = array() ){
		return $this->get_hnmgbox()->add_group( $field_args, $this );
	}

	public function get_group( $field_id = '' ){
		return $this->get_hnmgbox()->get_group( $field_id, $this );
	}

	public function open_mixed_field( $field_args = array() ){
		return $this->get_hnmgbox()->open_mixed_field( $field_args, $this );
	}

	public function close_mixed_field( $field_args = array() ){
		return $this->get_hnmgbox()->close_mixed_field( $field_args, $this );
	}

	public function add_tab( $field_args = array() ){
		return $this->get_hnmgbox()->add_tab( $field_args, $this );
	}

	public function close_tab( $field_id = '' ){
		return $this->get_hnmgbox()->close_tab( $field_id, $this );
	}

	public function open_tab_item( $item_name = '' ){
		return $this->get_hnmgbox()->open_tab_item( $item_name, $this );
	}

	public function close_tab_item( $item_name = '' ){
		return $this->get_hnmgbox()->close_tab_item( $item_name, $this );
	}

    public function add_html( $field_args = array() ){
        return $this->get_hnmgbox()->add_html( $field_args, $this );
    }
	...
	public function is_saved_field(){
		...
		if( $row_level == 1 ){
			switch( $this->get_hnmgbox()->get_object_type() ){
				case 'metabox':
					$saved = metadata_exists( 'post', $this->get_hnmgbox()->get_object_id(), $this->id );
					break;

				case 'admin-page':
					$options = get_option( $this->get_hnmgbox()->id );
	...
	public function get_saved_value(){
		...
		switch( $this->get_hnmgbox()->get_object_type() ){
			case 'metabox':
				$value = $this->get_hnmgbox()->get_field_value( $this->id, $this->get_hnmgbox()->get_object_id(), '' );
				break;

			case 'admin-page':
				$value = $this->get_hnmgbox()->get_field_value( $this->id, '' );
				break;
		}
		return $value;
	}
	...
	public function get_value( $default = true, $escaping_function = 'esc_attr', $index = null, $all = false ){
		...
		$row_level = $this->get_row_level();
		if( $row_level == 1 ){
			if( LB_Functions::is_empty( $this->value ) ){
		...
		if( LB_Functions::is_empty( $value ) && $default && ! $this->is_saved_field() ){
	...
	public function get_group_value(){
		...
		$group = $this->get_hnmgbox()->get_field( $parents['level-1']['id'] );
		if( $group->arg( 'type' ) == 'section' ){
			$group = $group->get_field( $parents['level-2']['id'] );
		}
		$value = $group->get_saved_value();

		if( is_array( $value ) && ! LB_Functions::is_empty( $value ) ){
			switch( $row_level ){
	...
	public function get_name( $index = null ){
		...
		//Para los campos dentro de grupos
			...
			$group_1 = $this->get_hnmgbox()->get_field( $parents['level-1']['id'] );
	...
	public function get_value_by_name( $name_attr = '' ){
		...
		$value = $group->get_saved_value();
		$value = LB_Functions::get_array_value_by_path( $field_name, $value );
		return $value;
	}
	...
	private function escape_value( $value = '', $escaping_function = 'esc_attr' ){
		...
		$escape = $this->get_hnmgbox()->exists_callback( 'escape_callback', $this );
	...
	public function save( $value = '' ){
		...
		switch( $this->get_hnmgbox()->get_object_type() ){
			case 'metabox':
				$return['updated'] = $this->get_hnmgbox()->set_field_value( $this->id, $new_value, $this->get_hnmgbox()->get_object_id() );
				break;

			case 'admin-page':
				$return['updated'] = $this->get_hnmgbox()->set_field_value( $this->id, $new_value );
				break;
		}
		return $return;
	}
	...
	public function sanitize_value( $value = '' ){
		...
		$sanitize = $this->get_hnmgbox()->exists_callback( 'sanitize_callback', $this );
		...
		$sanitizer = new LB_Sanitizer( $this, $value );
		$sanitized_value = $sanitizer->{$this->args['type']}();

		//Filtro para desinfección por el usuario según tipo de campo
		$filter_tag = "hnmgbox_sanitize_{$this->arg( 'type' )}";
		if( has_filter( $filter_tag ) ){
			$filter_value = apply_filters( $filter_tag, $sanitizer->value, $this->get_hnmgbox()->get_object_id(), $this->args, $sanitizer );
	...
	public function sanitize_group( $value = array(), $on_save = false ){
		...
		foreach ( (array) $value as $index => $array_values ){
			if( ! LB_Functions::is_empty( $array_values ) ){
				foreach ( (array) $array_values as $field_id => $val ){
					...
					if( $field ){
						if( $field->arg( 'type' ) == 'group' ){
						    ...
						    $val = apply_filters( "hnmgbox_filter_field_value_{$field_id}", $val );
	...
	public function get_result_callback( $callback = '' ){
		...
		if( isset( $this->args[ $callback ] ) && ! in_array( $this->args[ $callback ], $exclude ) && $this->get_hnmgbox()->exists_callback( $callback, $this )  ){

...
;(function (document, window, $){
  ...
  function Plugin (el, options){
    ...
    this.$nav    = this.$el.find('.hnmgbox-tab-nav').first();
    this.$links  = this.$nav.find('a');
    this.$body = this.$el.find('.hnmgbox-tab-body').first();
    this.$panels = this.$body.find('> .hnmgbox-tab-content');
  ...
  Plugin.prototype._setup = function ( $item_active ){
    if( ! $item_active ){
      ...
      if( $item_active.hasClass('hnmgbox-item-has-childs') ){
    ...
    this.$body.find('> .hnmgbox-tab-content[data-tab="'+_url+'"]').show();
  ...
  Plugin.prototype._setupAccordion = function (){
    ...
    this.$panels.each(function(i, el){ //for each tab panel
      ...
      var _class = '';
      if( $item.length ){
        _class = $item.attr('class').replace('hnmgbox-item', '');
        _class = _class.replace('active', '');
      }
      var data = $item.data('item') ? 'item="'+$item.data('item')+'"' : 'parent="'+$item.data('parent')+'"';

      self.$panels.eq(i).before('<h3 class="hnmgbox-accordion-title '+_class+'" data-'+ data+'><a href="' + _link + '"><span>' + _text + '</span></a></h3>'); //add the accordion title
    });

    this.$links = this.$links.add( this.$panels.parent().find('> .hnmgbox-accordion-title > a') ); //update the links variable after new items have been created
  };
  ...
  Plugin.prototype._events = function (){
    ...
    this.$links.on('click', function (event) { //on link click
      ...
      if( $(link).parent().hasClass('hnmgbox-item-has-childs') ){
        link = $(link).parent().next('.hnmgbox-item-child').find('a');
    ...
    this.$links.parent().on('click', '.hnmgbox-toggle-icon', function(event) {
      event.preventDefault();
      var $item = $(this).parent();
      var $toggle = $(this);
      var data_item = $item.data('item');
      if( $item.hasClass('hnmgbox-open') ){
        $item.removeClass('hnmgbox-open');
        $toggle.find('i').removeClass('hnmgbox-icon-chevron-up').addClass('hnmgbox-icon-chevron-down');
        $item.siblings('.hnmgbox-item-child').each(function(index, el) {
          if( $(el).data('parent') == data_item ){
            $(el).slideUp();
          }
        });
      } else {
        $item.addClass('hnmgbox-open');
        $item.find('a').trigger('click');
        $toggle.find('i').removeClass('hnmgbox-icon-chevron-down').addClass('hnmgbox-icon-chevron-up');
        $item.siblings('.hnmgbox-item-child').each(function(index, el) {
          if( $(el).data('parent') == data_item ){
            $(el).slideDown();
          }
  ...
  Plugin.prototype.openCloseChildItems = function (trigger){
    ...
    var data_item = $item.data('item');
    if( $item.hasClass('hnmgbox-accordion-title') ){
      return;
    }
    if( data_item ){
      var $toggle = $item.find('.hnmgbox-toggle-icon');
      $item.siblings('.hnmgbox-item-parent').removeClass('hnmgbox-open').find('.hnmgbox-toggle-icon i').removeClass('hnmgbox-icon-chevron-up').addClass('hnmgbox-icon-chevron-down');
  ...
  Plugin.prototype._change = function (trigger){
    ...
    if (!_trigger.parent().hasClass('active')) { //if the link is not already active
      ...
      $(document).trigger('hnmgbox-tabs.change', [this.$el, $(_newPanel)]);
  ...
  Plugin.prototype._initialise = function (){
    this.$el.addClass('hnmgbox-tabs hnmgbox-tabs-initialized');
    if( $.isFunction(this.options.initialised) ){
      this.options.initialised.call(this.$el);
    }
    $(document).trigger('hnmgbox-tabs.initialised', [this.$el]);

    //Fix first tab item with childs
    this.$links.eq(0).parent().find('.hnmgbox-toggle-icon').trigger('click');
    this.$links.eq(0).parent().find('.hnmgbox-toggle-icon').trigger('click');
  ...
  Plugin.prototype._accordion = function (panel){
    ...
    this.$body.find('>.hnmgbox-tab-content[data-tab="'+panel+'"]').stop(true, true).slideDown(this.options.speed);
  ...
  Plugin.prototype._accordionCollapse = function (panel){
    ...
    this.$body.find('>.hnmgbox-accordion-title > a[href="' + panel + '"]').parent().removeClass('active');

    this.$body.find('>.hnmgbox-tab-content[data-tab="'+panel+'"]').stop(true, true).slideUp(this.options.speed);
  };
  ...
  Plugin.prototype._tabs = function (panel){
    ...
    this.$body.find('>.hnmgbox-tab-content[data-tab="'+panel+'"]').show();
  };
  ...
  Plugin.prototype._updateActive = function (panel){
    ...
    this.$body.find('>.hnmgbox-accordion-title > a[href="' + panel + '"]').parent().addClass('active');
  };
  ...
  $.fn.hnmgboxTabs = function (options){
    var args = Array.prototype.slice.call(arguments, 1);

    return this.each(function (){
      var _this = $(this),
        _data = _this.data('hnmgbox-tabs');

      if (!_data){
        _this.data('hnmgbox-tabs', (_data = new Plugin(this, options)));
      }

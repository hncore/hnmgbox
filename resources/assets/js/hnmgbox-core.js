jQuery(document).ready(function(){
  ...
  setTimeout( function() {
    ...
    var $parent = jQuery($urlTrigger).parents('.hnmgbox-tab-content');
    if ($parent.length > 0) {
  ...
  jQuery(document).on('click', '.hnmgbox-item a', function(){
    var hash = jQuery(this).attr('href');
    updateUrlHash(hash);
  });

  jQuery(document).on('click', '.hnmgbox-sw-wrap', function(){
    toggleDisplayerOptions();
  });
